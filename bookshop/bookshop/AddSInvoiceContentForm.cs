...
namespace bookshop
{
    public partial class AddSInvoiceContentForm : Form
    {
        public NpgsqlConnection con;
        DataTable dt = new DataTable();
        DataSet ds = new DataSet();
        int iinvoice_id;
        public AddSInvoiceContentForm(NpgsqlConnection con, int iinvoice_id)
        {
            InitializeComponent();
            this.con = con;
        ...
        private void add_content_btn_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = "insert into sales_invoice_content (sinvoice_id, book_id, price, quantity) " +
                    "values (@sinvoice_id, @book_id, @price, @quantity)";
                using (NpgsqlCommand command = new NpgsqlCommand(sql, con))
                {
                    command.Parameters.Add("@sinvoice_id", NpgsqlTypes.NpgsqlDbType.Integer).Value = iinvoice_id;
        ...
        private void authot_cmbx_SelectedIndexChanged(object sender, EventArgs e)
        {
